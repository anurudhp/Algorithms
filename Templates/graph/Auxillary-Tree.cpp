...
int build(){ sort(aux.begin(), aux.end()); int len = aux.size();
	for(int i=1; i<len; ++i)
	  int x=LCA(aux[i].second,aux[i-1].second),aux.push_back({start[x], x});
	for(auto it:aux) is[it.second] = 0;
	vector<pair<int, int> >temp; temp.clear();
	for(auto it:aux){
		if(is[it.second]) continue; temp.push_back(it); is[it.second] = 1;
	} aux.clear(); for(auto it:temp) aux.push_back(it), is[it.second] = 0;
	...
	for(int i=1; i<aux.size(); ++i){
		...
		while(true){
			if(en[s.top()] < start[cur]) s.pop(); else break;
