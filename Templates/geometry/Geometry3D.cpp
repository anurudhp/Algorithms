...
public:
  ...
  bool operator < (point p) const{ if(eq(x,p.x) && eq(y,p.y))return z<p.z;
...
point tovect(point a,point b){return point(b.x-a.x,b.y-a.y,b.z-a.z);}
...
};bool areParallel(line l1,line l2){ return cross(l1.b,l2.b)==null &&
      !(cross(point(l1.a,l2.a),l2.b) == null);
}bool areSame(line l1,line l2){ return cross(l1.b,l2.b)==null &&
    (cross(point(l1.a,l2.a),l2.b) == null);
}bool areIntersect(line l1,line l2){ return !(cross(l1.b,l2.b)==null) &&
     (fabs(scalarTripleProduct(point(l1.a,l2.a),l1.b,l2.b))<EPS);
}bool areIntersect(line l1,line l2,point& p1){ if(!(cross(l1.b,l2.b)==null) &&
     (fabs(scalarTripleProduct(point(l1.a,l2.a),l1.b,l2.b))<EPS)){
...
}bool areSkew(line l1,line l2){ return !areParallel(l1,l2) && !areSame(l1,l2) &&
...
public:
  ...
  bool operator == (plane p) const{ return cross(n,p.n)==vect(0,0,0) &&
...
double ptPlaneDist(db x, db y, double z,double a, double b, double c, double d){
...
double ptLineDistSq(db x1, db y1, db z1, db x2, db y2, db z2, db px, double py,
		    double pz, int type){
...
}double ptLineDist(db x1, db y1, db z1,db x2, db y2, db z2, db px, db py, db pz,
...
point getProjection(point p,plane A){
  double a = A.n.x, b = A.n.y, c = A.n.z; double d = A.a.x, e = A.a.y, f = A.a.z;
...
bool inTriangle(point p,point A,point B,point C){
  ...
  double gamma = 1 - alpha - beta; return ok(alpha) && ok(beta) && ok(gamma);
...
point rotate(point A,point B,point C,double theta){
  double x = A.x, y = A.y, z = A.z; double a = B.x, b = B.y, c = B.z;
