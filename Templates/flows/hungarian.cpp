...
DBL hungarian(const VVD &cost,VI &Lmate,VI &Rmate){
  int n=SZ(cost); VD u(n), v(n);
  REP(i,n){ u[i]=cost[i][0];
    FOR(j,1,n) u[i]=min(u[i],cost[i][j]);
  }
  REP(j,n){ v[j]=cost[0][j]-u[0];
    FOR(i,1,n) v[j]=min(v[j],cost[i][j]-u[i]);
  }
  ...
  while(mated<n){
    ...
    u[s]+=dist[j];
    while(dad[j]>=0){
      const int d=dad[j];Rmate[j]=Rmate[d];
      Lmate[Rmate[j]]=j;j=d;
    }
    Rmate[j]=s;Lmate[s]=j;mated++;
  }
  DBL val=0; REP(i,n) val+=cost[i][Lmate[i]]; return val;
}