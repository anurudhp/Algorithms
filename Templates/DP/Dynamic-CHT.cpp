...
#define CLREF const Line&
  ...
  DBL meetX(CLREF l1, CLREF l2) {
    return parallel(l1, l2) ? INF : 1.0 * (l2.b - l1.b) / (l1.a - l2.a);
  } set<Line> hull;
  bool hasPrev(ITER it) { return it != hull.begin();
  } bool hasNext(ITER it) {
    return it != hull.end() && next(it) != hull.end();
  } bool bad(CLREF l1, CLREF l2, CLREF l3){ return meetX(l1,l3) <= meetX(l1,l2); }
  bool bad(ITER it) {
    return hasPrev(it) && hasNext(it) && (bad(*next(it), *it, *prev(it)));
  } ITER upd_lt_bord(ITER it) {
    ...
    it = upd_lt_bord(it);
    if(hasPrev(it)) upd_lt_bord(prev(it));
    if(hasNext(it)) upd_lt_bord(next(it));
  } LL eval(int x) {
